document.addEventListener('DOMContentLoaded', async function() {
    ...
    registerForm.addEventListener('submit', async function(event) {
        ...
        if (pwErrors.length > 0) {
            ...
            errorMessage.innerHTML = pwErrors.map(e => `• ${e}`).join('<br>');
            return;
        }
    ...
    function updatePasswordIndicators() {
        ...
        const local = (email && email.includes('@')) ? email.split('@')[0].toLowerCase() : '';
        ...
        setRule('notContainEmail', !(local && local.length >= 3 && pwd.toLowerCase().includes(local)));
        const common = ['password','123456','123456789','qwerty','12345678','111111','123123','abc123','letmein','iloveyou','admin'];
        ...
        if (confirmPasswordInput.value) {
            ...
            matchEl.textContent = matchOk ? 'Passwords match' : 'Passwords do not match';
    ...
    if (pwRulesBtn) {
        pwRulesBtn.addEventListener('click', function() {
            if (pwRulesPopup && pwRulesPopup.hasAttribute('hidden')) openPwPopup(); else closePwPopup();
        });
    }
    ...
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape' && pwRulesPopup && !pwRulesPopup.hasAttribute('hidden')) closePwPopup();
    });
